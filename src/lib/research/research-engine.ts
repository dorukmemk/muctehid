...
import { ResearchResult, ResearchFinding, ResearchSource, Contradiction, HallucinationReport } from '../../types/v2.js';
...
export class ResearchEngine {
  private searchFn: (query: string, k: number) => Promise<Array<{ id: string; content: string; filepath: string; score: number; language: string }>>;
  private docSearchFn: (query: string, k: number) => Promise<Array<{ id: string; content: string; filepath: string; score: number }>>;

  constructor(
    searchFn: ResearchEngine['searchFn'],
    docSearchFn: ResearchEngine['docSearchFn'],
  ) {
    this.searchFn = searchFn;
    this.docSearchFn = docSearchFn;
  }

  async research(topic: string, opts: ResearchOptions = {}): Promise<ResearchResult> {
    ...
    if (contradictions.length > 0) caveats.push(`${contradictions.length} contradiction(s) found in sources.`);
    if (hallucinationReport.unverifiedClaims.length > 0) caveats.push(`${hallucinationReport.unverifiedClaims.length} unverified claim(s) flagged.`);
    if (allFindings.length < 3) caveats.push('Limited evidence found — results may be incomplete.');
...
function decomposeQuery(topic: string): string[] {
...
function markCorroborated(findings: ResearchFinding[]): void {
  for (let i = 0; i < findings.length; i++) {
    for (let j = i + 1; j < findings.length; j++) {
      if (findings[i].source.id === findings[j].source.id) continue;
...
function detectContradictions(findings: ResearchFinding[]): Contradiction[] {
...
function synthesize(topic: string, findings: ResearchFinding[]): string {
  if (findings.length === 0) return `No relevant information found for: ${topic}`;
...
function checkHallucinations(synthesis: string, findings: ResearchFinding[]): HallucinationReport {
  ...
  for (const sentence of sentences) {
    ...
    if (hasEvidence) verified.push(sentence);
    else unverified.push(sentence);
  }
