...
export class CognitiveEngine {
  constructor(
    ...
    private timeline: TimelineMemory,
    private fileNotes: FileNotes,
    private facts: ImportantFacts,
    private graphStore: GraphStore | null,
    private repoRoot: string,
  ) {}

  async recallForFile(fp: string): Promise<ThinkResult> {
    const warns: string[] = [];
    const sugs: string[] = [];
    const mems: string[] = [];
    const parts: string[] = [];
    ...
    if (notes.length > 0) {
      ...
      for (const n of notes) {
        ...
        parts.push(`[${e}] ${n.note}`);
        if (n.category === 'warning') warns.push(n.note);
      }
    }
    ...
    if (this.graphStore) {
      try {
        const syms = await this.graphStore.query(
          'SELECT name, kind FROM symbols WHERE filepath = ? OR filepath LIKE ?',
          [fp, '%' + path.basename(fp)]
        );
        if (syms.length > 0) {
          parts.push('\n## Semboller (' + syms.length + ')');
          for (const s of syms.slice(0, 10)) parts.push('- ' + s.kind + ': ' + s.name);
        }
    ...
    try {
      ...
      if (rf.length > 0) {
        parts.push('\n## Ilgili Bilgiler');
        for (const f of rf) parts.push('[' + f.importance + '] ' + f.fact);
      }
    } catch { /* no embeddings */ }

    const drift = this.working.checkDrift();
    if (drift) warns.push(drift);
    this.working.touchFile(fp);

    return { context: parts.join('\n'), warnings: warns, suggestions: sugs, relatedMemories: mems };
  }

  async predictChange(fp: string, desc: string): Promise<ChangeImpact> {
    const af: string[] = [];
    const as2: string[] = [];
    const w: string[] = [];
    const sg: string[] = [];
    ...
    if (this.graphStore) {
      try {
        const syms = await this.graphStore.query(
          'SELECT uid, name FROM symbols WHERE filepath = ? OR filepath LIKE ?',
          [fp, '%' + path.basename(fp)]
        );
        for (const sym of syms) {
          const callers = await this.graphStore.query(
            'SELECT DISTINCT s.filepath, s.name FROM relations r JOIN symbols s ON r.fromUid = s.uid WHERE r.toUid = ?',
            [sym.uid]
          );
          for (const c of callers) {
            if (!af.includes(c.filepath)) af.push(c.filepath);
            as2.push(c.name + ' (calls ' + sym.name + ')');
          }
        }
        if (af.length >= 10) risk = 'critical';
        else if (af.length >= 5) risk = 'high';
        else if (af.length >= 2) risk = 'medium';
      } catch { /* */ }
    }

    const notes = this.fileNotes.get(fp);
    for (const n of notes) {
      if (n.category === 'warning') w.push(n.note);
    }

    const failed = await this.timeline.search({ query: fp, outcome: 'failure', limit: 3 });
    if (failed.length > 0) w.push('Bu dosyada ' + failed.length + ' basarisiz islem gecmisi var.');

    const similar = await this.timeline.search({ query: desc, limit: 3 });
    if (similar.length > 0) {
      sg.push('Benzer gecmis:');
      for (const e of similar) sg.push('  [' + e.outcome + '] ' + e.action);
    }

    if (af.length > 0) sg.push(af.length + ' dosya etkilenecek');

    return { file: fp, affectedFiles: af, affectedSymbols: as2, riskLevel: risk, warnings: w, suggestions: sg };
  }

  async recallExperience(task: string): Promise<string> {
    const lines: string[] = [];
    ...
    if (similar.length > 0) {
      ...
      for (const e of similar) {
        ...
        if (e.context) lines.push('   Baglam: ' + e.context);
        if (e.files && e.files.length > 0) lines.push('   Dosyalar: ' + e.files.join(', '));
      }
    }
    ...
    try {
      ...
      if (rf.length > 0) {
        lines.push('\n## Ilgili Bilgiler');
        for (const f of rf) lines.push('[' + f.importance + '] ' + f.fact);
      }
    } catch { /* */ }
    ...
    try {
      ...
      if (rn.length > 0) {
        lines.push('\n## Ilgili Notlar');
        for (const n of rn) lines.push(n.filepath + ': ' + n.note);
      }
    } catch { /* */ }
  ...
  suggestNextAction(): string {
    ...
    if (!ctx.activeGoal && !ctx.currentTask) return 'Aktif gorev yok. task_next ile sonraki gorevi al.';
    const lines: string[] = [];
    if (ctx.currentTask) lines.push('Aktif gorev: ' + ctx.currentTask);
    if (ctx.activeGoal) lines.push('Hedef: ' + ctx.activeGoal);
    const drift = this.working.checkDrift();
    if (drift) lines.push(drift);
    return lines.join('\n');
  }

  async getSessionBriefing(): Promise<string> {
    const lines: string[] = [];

    const topFacts = this.facts.getTopFacts(5);
    if (topFacts.length > 0) {
      lines.push('## Onemli Bilgiler');
      for (const f of topFacts) lines.push('[' + f.importance + '/' + f.category + '] ' + f.fact);
      lines.push('');
    }
    ...
    if (recent.length > 0) {
      ...
      for (const ev of recent) lines.push('[' + ev.outcome + '] ' + this.ago(ev.timestamp) + ': ' + ev.action);
      lines.push('');
    }
    ...
    if (todos.length > 0) {
      ...
      for (const t of todos.slice(0, 5)) lines.push('- ' + t.filepath + ': ' + t.note);
      lines.push('');
    }
    ...
    if (warns.length > 0) {
      ...
      for (const ww of warns.slice(0, 5)) lines.push('- ' + ww.filepath + ': ' + ww.note);
    }
