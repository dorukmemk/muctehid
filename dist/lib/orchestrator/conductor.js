...
const complexity_detector_js_1 = require("./complexity-detector.js");
const prompt_builder_js_1 = require("./prompt-builder.js");
// ─── Step Builder Helpers ─────────────────────────────────────────────────────
function makeStep(order, tool, args, description, dependsOn = [], miniPrompt) {
    return {
...
function buildStepPlan(intent, repoRoot) {
    ...
    switch (analysis.level) {
        case 'trivial':
            strategy = 'direct';
            steps.push(makeStep(order++, 'search_code', { query: intent, k: 3 }, 'Quick code search', [], `## Step: Quick code search
...
**On failure:** call orch_report with error describing why search failed.`));
        ...
        case 'simple':
            strategy = 'direct';
            steps.push(makeStep(order++, 'search_code', { query: intent, k: 5 }, 'Search relevant code', [], `## Step: Search relevant code
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'get_context', { filepath: repoRoot }, 'Load project context', [], `## Step: Load project context
...
**On failure:** call orch_report with error.`));
        ...
        case 'moderate':
            strategy = 'skill';
            steps.push(makeStep(order++, 'search_code', { query: intent, k: 8 }, 'Search relevant code', [], `## Step: Search relevant code
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'research_topic', { topic: intent }, 'Research the topic', [], `## Step: Research the topic
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'generate_report', { title: `Report: ${intent.slice(0, 60)}`, repoRoot, save: true }, 'Generate and save report', [], `## Step: Generate report
...
**On failure:** call orch_report with error.`));
        ...
        case 'complex':
            ...
            steps.push(makeStep(order++, 'search_code', { query: intent, k: 10 }, 'Broad code search', [], `## Step: Broad code search
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'search_code', { query: `${intent} types interfaces`, k: 5 }, 'Search types and interfaces', [], `## Step: Search types and interfaces
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'research_topic', { topic: intent }, 'Deep research', [], `## Step: Deep research
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'run_skill', { skill: 'deep-dive', path: repoRoot }, 'Deep dive analysis', [], `## Step: Deep dive analysis
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'run_skill', { skill: 'audit-runner', path: repoRoot }, 'Audit runner', [], `## Step: Audit runner
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'task_next', {}, 'Check next priority task', [], `## Step: Check next priority task
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'generate_report', { title: `Analysis: ${intent.slice(0, 60)}`, repoRoot, save: true }, 'Generate and save final report', [], `## Step: Generate final report
...
**On failure:** call orch_report with error.`));
        ...
        case 'epic':
            strategy = 'spec';
            steps.push(makeStep(order++, 'spec_init', { name: intent.slice(0, 60), description: intent }, 'Initialize spec', [], `## Step: Initialize spec
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'spec_generate', { phase: 'requirements' }, 'Generate requirements', [], `## Step: Generate requirements
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'spec_generate', { phase: 'design' }, 'Generate design', [], `## Step: Generate design
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'spec_generate', { phase: 'tasks' }, 'Generate tasks', [], `## Step: Generate tasks
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'search_code', { query: intent, k: 10 }, 'Search existing code', [], `## Step: Search existing code
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'research_topic', { topic: intent }, 'Research context', [], `## Step: Research context
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'run_skill', { skill: 'deep-dive', path: repoRoot }, 'Deep dive', [], `## Step: Deep dive
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'run_skill', { skill: 'impact-analyzer', path: repoRoot }, 'Impact analysis', [], `## Step: Impact analysis
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'run_skill', { skill: 'refactor-planner', path: repoRoot }, 'Refactor planning', [], `## Step: Refactor planning
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'run_skill', { skill: 'audit-runner', path: repoRoot }, 'Audit', [], `## Step: Audit
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'task_next', {}, 'Surface first task to action', [], `## Step: Surface first task
...
**On failure:** call orch_report with error.`));
            steps.push(makeStep(order++, 'generate_report', { title: `Epic: ${intent.slice(0, 60)}`, repoRoot, save: true }, 'Generate final report', [], `## Step: Generate final report
...
**On failure:** call orch_report with error.`));
    ...
    if (isBug) {
        ...
        steps.push(makeStep(order++, 'search_code', { query: intent, k: 8 }, 'Find relevant code for bug', [], `## Step: Find relevant code for bug
...
**On failure:** call orch_report with error.`));
        steps.push(makeStep(order++, 'get_context', { filepath: repoRoot }, 'Load full file context for bug location', [], `## Step: Load full file context
...
**On failure:** call orch_report with error.`));
        steps.push(makeStep(order++, 'audit_file', { filepath: repoRoot }, 'Audit file for bug root cause', [], `## Step: Audit file for bug root cause
...
**On failure:** call orch_report with error.`));
        steps.push(makeStep(order++, 'run_skill', { skill: 'bug-reporter', path: repoRoot }, 'Run bug reporter for full analysis', [], `## Step: Run bug reporter
...
**On failure:** call orch_report with error.`));
        steps.push(makeStep(order++, 'generate_report', { title: `Bug Report: ${intent.slice(0, 50)}`, repoRoot, save: true }, 'Save bug report', [], `## Step: Save bug report
...
**On failure:** call orch_report with error.`));
    ...
    if (isSecurity) {
        strategy = 'skill';
        steps.push(makeStep(order++, 'index_codebase', { repoRoot }, 'Index codebase for security scan', [], `## Step: Index codebase
...
**On failure:** call orch_report with error (non-fatal — continue to next step).`));
        steps.push(makeStep(order++, 'find_secrets', { path: repoRoot }, 'Scan for hardcoded secrets', [], `## Step: Scan for hardcoded secrets
...
**On failure:** call orch_report with error.`));
        steps.push(makeStep(order++, 'security_scan', { path: `${repoRoot}/src` }, 'Full security scan of src/', [], `## Step: Full security scan
...
**On failure:** call orch_report with error.`));
        steps.push(makeStep(order++, 'run_skill', { skill: 'security-audit', path: repoRoot }, 'Deep security audit', [], `## Step: Deep security audit
...
**On failure:** call orch_report with error.`));
        steps.push(makeStep(order++, 'generate_report', { title: `Security Audit: ${intent.slice(0, 50)}`, repoRoot, save: true }, 'Save security report', [], `## Step: Save security report
...
**On failure:** call orch_report with error.`));
    ...
    if (isRefactor) {
        ...
        steps.push(makeStep(order++, 'complexity_score', { filepath: repoRoot }, 'Measure code complexity', [], `## Step: Measure code complexity
...
**On failure:** call orch_report with error.`));
        steps.push(makeStep(order++, 'run_skill', { skill: 'impact-analyzer', path: repoRoot }, 'Analyze refactor impact', [], `## Step: Analyze refactor impact
...
**On failure:** call orch_report with error.`));
        steps.push(makeStep(order++, 'run_skill', { skill: 'refactor-planner', path: repoRoot }, 'Plan refactoring steps', [], `## Step: Plan refactoring steps
...
**On failure:** call orch_report with error.`));
    ...
    if (isFeature && analysis.level !== 'epic') {
        strategy = 'spec';
        steps.push(makeStep(order++, 'spec_init', { name: intent.slice(0, 60), description: intent }, 'Initialize feature spec', [], `## Step: Initialize feature spec
...
**On failure:** call orch_report with error.`));
        steps.push(makeStep(order++, 'spec_generate', { phase: 'requirements' }, 'Generate feature requirements', [], `## Step: Generate feature requirements
...
**On failure:** call orch_report with error.`));
        steps.push(makeStep(order++, 'spec_generate', { phase: 'design' }, 'Generate feature design', [], `## Step: Generate feature design
...
**On failure:** call orch_report with error.`));
        steps.push(makeStep(order++, 'spec_generate', { phase: 'tasks' }, 'Generate implementation tasks', [], `## Step: Generate implementation tasks
...
**On failure:** call orch_report with error.`));
    ...
    if (isTest) {
        steps.push(makeStep(order++, 'run_skill', { skill: 'test-generator', path: repoRoot }, 'Generate tests', [], `## Step: Generate tests
...
**On failure:** call orch_report with error.`));
    ...
    if (isDocs) {
        steps.push(makeStep(order++, 'run_skill', { skill: 'doc-generator', path: repoRoot }, 'Generate documentation', [], `## Step: Generate documentation
...
**On failure:** call orch_report with error.`));
    ...
    if (isResearch && !steps.some(s => s.tool === 'research_topic')) {
        steps.push(makeStep(order++, 'research_topic', { topic: intent }, 'Research topic', [], `## Step: Research topic
...
class Conductor {
    sessionStore;
    taskStore;
    constructor(sessionStore, taskStore) {
        this.sessionStore = sessionStore;
        this.taskStore = taskStore;
    }
    start(intent, repoRoot) {
        ...
        const prompt = (0, prompt_builder_js_1.buildOrchestratorPrompt)(freshSession, pendingTasks, recentEvents);
        return { session: freshSession, prompt, firstStep };
    }
    report(sessionId, type, data) {
        ...
        if (type === 'task' && data.taskData) {
            ...
            const priority = ['critical', 'high', 'medium', 'low'].includes(td.priority)
                ? td.priority
                : 'medium';
            const category = ['feature', 'bug', 'refactor', 'docs', 'test', 'research', 'chore'].includes(td.category)
                ? td.category
                : 'chore';
        ...
        const prompt = (0, prompt_builder_js_1.buildOrchestratorPrompt)(session, pendingTasks, recentEvents);
    ...
    autoCreateBugTasks(sessionId, issues) {
        const createdIds = [];
