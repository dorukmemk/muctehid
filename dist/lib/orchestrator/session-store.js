...
class SessionStore {
    ...
    constructor(dataDir) {
        ...
        this.db = new better_sqlite3_1.default(path.join(dataDir, 'orchestrator.db'));
    ...
    createSession(intent, strategy, steps) {
    ...
    getSession(id) {
        const row = this.db.prepare('SELECT * FROM orch_sessions WHERE id = ?').get(id);
        if (!row)
            return null;
        return this.rowToSession(row);
    }
    getActive() {
        const row = this.db.prepare("SELECT * FROM orch_sessions WHERE status = 'active' ORDER BY updated_at DESC LIMIT 1").get();
        if (!row)
            return null;
    ...
    cleanupStale() {
        ...
        const result = this.db.prepare("UPDATE orch_sessions SET status = 'cancelled', updated_at = ? WHERE status = 'active' AND updated_at < ?").run(Date.now(), cutoff);
    ...
    markStepDone(sessionId, stepId, result) {
        const session = this.getSession(sessionId);
        if (!session)
            return;
        const steps = session.steps.map(s => s.id === stepId ? { ...s, status: 'done', result } : s);
    ...
    markStepFailed(sessionId, stepId, error) {
        const session = this.getSession(sessionId);
        if (!session)
            return;
        const steps = session.steps.map(s => s.id === stepId ? { ...s, status: 'failed', result: error } : s);
        const now = Date.now();
    ...
    addTaskCreated(sessionId, taskId) {
        const session = this.getSession(sessionId);
        if (!session)
            return;
    ...
    nextPendingStep(sessionId) {
        const session = this.getSession(sessionId);
        if (!session)
            return null;
        return session.steps.find(s => s.status === 'pending') ?? null;
    }
    getEvents(sessionId, limit = 20) {
        const rows = this.db.prepare('SELECT * FROM orch_events WHERE session_id = ? ORDER BY timestamp DESC LIMIT ?').all(sessionId, limit);
    ...
    getRecentEvents(sessionId, limit) {
        const rows = this.db.prepare('SELECT * FROM orch_events WHERE session_id = ? ORDER BY timestamp DESC LIMIT ?').all(sessionId, limit);
        return rows.reverse().map(r => ({
            id: r.id,
            sessionId: r.session_id,
    ...
    listSessions(limit = 20) {
        const rows = this.db.prepare('SELECT * FROM orch_sessions ORDER BY updated_at DESC LIMIT ?').all(limit);
    ...
    rowToSession(row) {
        return {
            ...
            tasksCreated: JSON.parse(row.tasks_created || '[]'),
            context: row.context || '',
