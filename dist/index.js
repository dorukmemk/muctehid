...
const code_memory_js_1 = require("./lib/memory/code-memory.js");
const registry_js_1 = require("./lib/skills/registry.js");
...
const registry_js_2 = require("./lib/plugins/registry.js");
...
const session_store_js_1 = require("./lib/orchestrator/session-store.js");
const conductor_js_1 = require("./lib/orchestrator/conductor.js");
const orchestrator_js_1 = require("./tools/orchestrator.js");
const deep_reporter_js_1 = require("./lib/reporter/deep-reporter.js");
const memory_tools_js_1 = require("./tools/memory-tools.js");
...
// ─── Load config ──────────────────────────────────────────────────────────────
const config = (0, config_js_1.loadConfig)(REPO_ROOT);
// ─── Initialize services ──────────────────────────────────────────────────────
let memory = null;
async function getMemory() {
    if (!memory) {
        memory = await code_memory_js_1.CodeMemory.open(AUDIT_DATA_DIR);
    }
    return memory;
}
const skillRegistry = new registry_js_1.SkillRegistry([BUILTIN_SKILLS_DIR, INSTALLED_SKILLS_DIR]);
...
(0, memory_tools_js_1.setMemoryDeps)(null, REPO_ROOT); // Graph store set later when graph_build runs
...
const conductor = new conductor_js_1.Conductor(sessionStore, taskStore);
const specEngine = new spec_engine_js_1.SpecEngine(AUDIT_DATA_DIR);
const templateRegistry = new template_registry_js_1.TemplateRegistry([
    BUILTIN_TEMPLATES_DIR,
    path.join(REPO_ROOT, '.templates'),
]);
// Lazy ResearchEngine (needs memory)
let researchEngine = null;
async function getResearchEngine() {
    if (!researchEngine) {
        const mem = await getMemory();
        researchEngine = new research_engine_js_1.ResearchEngine(async (query, k) => {
...
if (config.plugins.length > 0) {
    pluginRegistry.load(config.plugins, REPO_ROOT).catch(e => console.error('[muctehid-mcp] Plugin load error:', e));
}
// ─── MCP Server ───────────────────────────────────────────────────────────────
const server = new index_js_1.Server({ name: 'muctehid-mcp', version: '2.0.0' }, { capabilities: { tools: {} } });
// ─── Tool Definitions ─────────────────────────────────────────────────────────
const TOOLS = [
    // ── Memory (6) ────────────────────────────────────────────────────────────
    ...
    // ── Orchestrator (8) ──────────────────────────────────────────────────────
    ...orchestrator_js_1.ORCHESTRATOR_TOOL_DEFS,
    // ── Spec / Kiro-mode (5) ──────────────────────────────────────────────────
    ...
    { name: 'graph_query', description: 'Execute query on knowledge graph. Accepts SQLite SQL or basic Cypher patterns (auto-translated). Tables: symbols(uid,name,kind,filepath,startLine,endLine), relations(fromUid,toUid,type,confidence). Examples: "SELECT name,kind FROM symbols WHERE name LIKE \'%User%\'" or "MATCH (fn:Function) RETURN fn".', inputSchema: { type: 'object', required: ['query'], properties: { query: { type: 'string', description: 'SQL query or basic Cypher pattern' } } } },
];
    ...
    try {
        ...
        if (['index_codebase', 'search_code', 'add_memory', 'get_context', 'memory_stats', 'clear_memory'].includes(name)) {
            ...
            switch (name) {
                ...
                case 'search_code': {
                    const results = await mem.search(args.query, {
                        k: args.k ?? 10,
        ...
        // ── Enhanced Memory tools ─────────────────────────────────────────────────
            ...
            const result = await tool.handler(args, AUDIT_DATA_DIR);
            text = result.content[0].text;
        }
        // ── Audit tools ───────────────────────────────────────────────────────────
        else if (name === 'audit_file') {
            const result = (0, audit_js_1.auditFile)(args.filepath);
            text = result.markdown;
            text += `\n\n---\n> 💾 Detaylı rapor için: \`generate_report filepath="${args.filepath}" save=true\``;
        }
        else if (name === 'audit_diff') {
            text = await gitTools.handleTool('git_diff_audit', args);
        }
        else if (name === 'security_scan') {
            const { scanSecurity } = await Promise.resolve().then(() => __importStar(require('./lib/audit/security.js')));
            const target = args.path;
            const files = collectFiles(target, ['.ts', '.js', '.tsx', '.jsx', '.py', '.php', '.rb', '.go', '.java', '.cs']);
            const allIssues = files.flatMap(f => {
                try {
                    return scanSecurity(f, fs.readFileSync(f, 'utf-8'));
                }
                catch {
                    return [];
                }
            ...
            const allSecrets = files.flatMap(f => {
                try {
                    return (0, secrets_js_1.scanSecrets)(f, fs.readFileSync(f, 'utf-8'));
                }
                catch {
                    return [];
                }
            ...
            const allTodos = files.flatMap(f => {
                try {
                    return (0, quality_js_1.scanTodos)(f, fs.readFileSync(f, 'utf-8')).map(t => ({ ...t, filepath: f }));
                }
                catch {
                    return [];
                }
            ...
            text = allTodos.length === 0
                ...
                : `## TODOs (${allTodos.length}) across ${files.length} files\n\n` +
                    allTodos.map(t => `- **[${t.type}]** \`${path.relative(process.cwd(), t.filepath)}:${t.line}\` — ${t.text}`).join('\n');
        }
        else if (name === 'complexity_score') {
            const issues = (0, complexity_js_1.complexityIssues)(args.filepath);
            ...
            text = await (0, health_js_1.getHealthScore)(REPO_ROOT);
        ...
        // ── Skills tools ──────────────────────────────────────────────────────────
            ...
            if (name === 'run_skill') {
                ...
                if (save) {
                    ...
                    const savedPath = (0, deep_reporter_js_1.saveReport)(text, AUDIT_DATA_DIR, reportName);
                    text = `✅ Rapor kaydedildi: \`${savedPath}\`\n\n${text}`;
                }
                else {
                    const targetHint = args.filepath ?? args.path ?? '';
        ...
        // ── Report tools ──────────────────────────────────────────────────────────
            ...
            if (name === 'generate_report') {
                ...
                if (save) {
                    const reportContent = (0, deep_reporter_js_1.buildReport)({
                    ...
                    const savedPath = (0, deep_reporter_js_1.saveReport)(reportContent, AUDIT_DATA_DIR, 'audit');
                    text = `✅ Rapor kaydedildi: \`${savedPath}\`\n\n${reportContent}`;
                }
                else {
                    text = baseReport;
        ...
        // ── Context tools ─────────────────────────────────────────────────────────
            ...
            const searchRoot = args.path ?? REPO_ROOT;
            const findings = [];
        ...
        // ── Orchestrator tools ────────────────────────────────────────────────────
        else if (orchestrator_js_1.ORCHESTRATOR_TOOL_NAMES.includes(name)) {
            text = await (0, orchestrator_js_1.handleOrchestratorTool)(name, args, conductor, skillRegistry, REPO_ROOT);
        }
        // ── Spec tools ────────────────────────────────────────────────────────────
        else if (name === 'spec_init') {
            const spec = specEngine.create(args.name, args.description, REPO_ROOT);
            ...
            if (specs.length === 0) {
                text = 'Hiç spec yok. `spec_init` ile oluşturun.';
            }
            else {
                text = specs.map(s => `**${s.id}** — ${s.name}\n  Status: ${s.status} | Tasks: ${s.taskIds.length}\n  ${new Date(s.createdAt).toISOString().slice(0, 10)}`).join('\n\n');
            }
        }
        else if (name === 'spec_get') {
            const spec = specEngine.get(args.specId);
            if (!spec)
                throw new Error(`Spec bulunamadı: ${args.specId}`);
            text = JSON.stringify(spec, null, 2);
        }
        else if (name === 'spec_update_status') {
            specEngine.updateStatus(args.specId, args.status);
            ...
            if (!spec)
                throw new Error(`Spec bulunamadı: ${args.specId}`);
            const providedContent = args.content;
            switch (args.phase) {
                case 'requirements': {
                    if (providedContent) {
                        specEngine.writeRequirements(args.specId, providedContent);
                        text = `✅ **requirements.md** kaydedildi\n\n📋 \`${spec.requirementsPath}\`\n\nSonraki: \`spec_generate specId="${args.specId}" phase="design"\``;
                    }
                    else {
                        ...
                        try {
                            ...
                            if (results.length > 0) {
                                codebaseCtx = results.map(r => `// ${path.relative(REPO_ROOT, r.chunk.filepath)}:${r.chunk.startLine}\n${r.chunk.content.slice(0, 250)}`).join('\n\n');
                            }
                        }
                        ...
                        text = buildRequirementsPrompt(spec.name, spec.description, args.specId, codebaseCtx, topFiles);
                    }
                    break;
                }
                case 'design': {
                    if (providedContent) {
                        specEngine.writeDesign(args.specId, providedContent);
                        text = `✅ **design.md** kaydedildi\n\n🏗️ \`${spec.designPath}\`\n\nSonraki: \`spec_generate specId="${args.specId}" phase="tasks"\``;
                    }
                    else {
                        ...
                        const patterns = [];
                        if (srcFiles.some(f => f.includes('repository')))
                            patterns.push('Repository pattern mevcut');
                        if (srcFiles.some(f => f.includes('/hooks/')))
                            patterns.push('React hooks kullanılıyor');
                        if (srcFiles.some(f => f.includes('/api/')))
                            patterns.push('Next.js API routes mevcut');
                        if (fs.existsSync(path.join(REPO_ROOT, 'prisma', 'schema.prisma')))
                            patterns.push('Prisma ORM kullanılıyor');
                        if (srcFiles.some(f => f.includes('middleware')))
                            patterns.push('Middleware katmanı mevcut');
                        if (fs.existsSync(path.join(REPO_ROOT, 'drizzle.config.ts')) || fs.existsSync(path.join(REPO_ROOT, 'drizzle.config.js')))
                            patterns.push('Drizzle ORM kullanılıyor');
                        const archContext = patterns.join(', ');
                        text = buildDesignPrompt(spec.name, args.specId, reqContent, spec.designPath, archContext);
                    }
                    break;
                }
                case 'tasks': {
                    if (providedContent) {
                        specEngine.writeTasks(args.specId, providedContent);
                        ...
                        const designContent = fs.existsSync(spec.designPath)
                            ...
                            : '';
                        text = buildTasksPrompt(spec.name, args.specId, reqContent, designContent, spec.tasksPath);
                    }
                    break;
                }
                default:
        ...
        // ── Task tools ────────────────────────────────────────────────────────────
            ...
            if (tasks.length === 0) {
                ...
                text = tasks.map(t => `**${t.id}** [${t.priority.toUpperCase()}] ${t.title}\n  Status: ${t.status} | ${t.estimateHours ?? '?'}h | ${t.category}`).join('\n\n');
            }
        }
        else if (name === 'task_get') {
            const task = taskStore.getById(args.taskId);
            if (!task)
                throw new Error(`Görev bulunamadı: ${args.taskId}`);
            text = JSON.stringify(task, null, 2);
        }
        else if (name === 'task_update') {
            const task = taskStore.update(args.taskId, {
            ...
            const ok = taskStore.delete(args.taskId);
            text = ok ? `✅ Silindi: ${args.taskId}` : `Görev bulunamadı: ${args.taskId}`;
        }
        else if (name === 'task_timeline') {
            const timeline = taskStore.getTimeline(args.taskId);
            if (timeline.length === 0) {
                text = 'Timeline boş.';
            }
            else {
                text = timeline.map(e => `[${new Date(e.timestamp).toISOString()}] **${e.event}**${e.detail ? `: ${e.detail}` : ''}`).join('\n');
            }
        }
            ...
            if (top.length === 0) {
                text = 'Yapılabilir görev yok.';
            }
            else {
                text = top.map((t, i) => `${i + 1}. **${t.id}** [${t.priority}] ${t.title}\n   ${t.estimateHours ?? '?'}h — ${t.description.slice(0, 80)}`).join('\n\n');
            }
        }
        ...
        // ── Research tools ────────────────────────────────────────────────────────
            ...
            const directHits = [];
            for (const file of srcFiles) {
                if (directHits.length >= maxSources)
                    break;
                try {
                    const lines = fs.readFileSync(file, 'utf-8').split('\n');
                    for (let i = 0; i < lines.length; i++) {
            ...
            const eng = await getResearchEngine();
            const result = await eng.research(args.claim, { maxSources: 10 });
        ...
        // ── Template tools ────────────────────────────────────────────────────────
            ...
            if (templates.length === 0) {
                ...
                text = templates.map(t => `**${t.name}** (v${t.version})\n  ${t.description}\n  Değişkenler: ${t.variables.join(', ')}`).join('\n\n');
            }
        }
        else if (name === 'template_render') {
            const tmpl = templateRegistry.get(args.templateName);
            if (!tmpl)
                throw new Error(`Şablon bulunamadı: ${args.templateName}`);
            const content = fs.readFileSync(tmpl.path, 'utf-8');
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            text = (0, template_engine_js_1.render)(content, args.variables ?? {});
        }
...
function buildRequirementsPrompt(name, description, specId, codebaseCtx, repoFiles) {
...
function buildDesignPrompt(name, specId, requirementsContent, _designPath, archContext) {
...
function buildTasksPrompt(name, specId, requirementsContent, designContent, _tasksPath) {
...
function _DELETED_buildDesignPrompt_OLD(name, specId, requirementsContent, _designPath, archContext) {
...
function _DELETED_buildTasksPrompt_OLD(name, specId, requirementsContent, designContent, _tasksPath) {
